...
class ilEtherpadLiteQuests
{
	...
	function addQuest()
	{
		...
		return $ilDB->manipulate("INSERT INTO rep_robj_xpdl_quests (pad_id, username, quest) VALUES (" .
	...
	{
		global $ilDB;
		$result = $ilDB->query("SELECT quest_id FROM rep_robj_xpdl_quests WHERE pad_id = " . $ilDB->quote($this->getPadId(), "text"));
	...
	{
		global $ilDB;
		$result = $ilDB->query("SELECT username, quest, created_at FROM rep_robj_xpdl_quests WHERE pad_id = " . $ilDB->quote($this->getPadId(), "text"));
		$rows = array();
	...
	public function revokeQuestions(){
		global $ilDB;
		return $ilDB->manipulate("DELETE FROM rep_robj_xpdl_quests
